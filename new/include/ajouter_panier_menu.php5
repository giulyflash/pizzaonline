...
else
{
	...
	$_SESSION['panier']['menu'][ $idmenu ]['nommenu']=$_POST['nommenu'];
	$_SESSION['panier']['menu'][ $idmenu]['prix']=$_POST['prix'];
	if(empty($_SESSION['panier']['menu'][ $idmenu ]['quantite'])){
		$_SESSION['panier']['menu'][ $idmenu ]['quantite']=$_POST['quantite'];
	}else $_SESSION['panier']['menu'][ $idmenu ]['quantite'] += $_POST['quantite'];
	if(!empty($_POST['nbgalette'])){
		$_SESSION['panier']['menu'][ $idmenu ]['nbgalette'] = intval($_POST['nbgalette']);
		for($i=0;$i<$_POST['nbgalette'];$i++){
			$_SESSION['panier']['menu'][ $idmenu ]['galette'.($i+1)]=$_POST['galette'.($i+1)];
		}
	}
	if(!empty($_POST['nbcrepe'])){
		$_SESSION['panier']['menu'][$idmenu ]['nbcrepe'] = intval($_POST['nbcrepe']);
		for($i=0;$i<$_POST['nbcrepe'];$i++){
			$_SESSION['panier']['menu'][ $idmenu ]['crepe'.($i+1)]=$_POST['crepe'.($i+1)];
		}
	}
	if(!empty($_POST['nbdessert'])){
		$_SESSION['panier']['menu'][ $idmenu ]['nbdessert'] = intval($_POST['nbdessert']);
		for($i=0;$i<$_POST['nbdessert'];$i++){
			$_SESSION['panier']['menu'][ $_POST['idmenu'] ]['dessert'.($i+1)]=$_POST['dessert'.($i+1)];
		}
	}
	if(!empty($_POST['nbboisson'])){
		$_SESSION['panier']['menu'][ $idmenu ]['nbboisson'] = intval($_POST['nbboisson']);
		for($i=0;$i<$_POST['nbboisson'];$i++){
			$_SESSION['panier']['menu'][ $idmenu]['boisson'.($i+1)]=$_POST['boisson'.($i+1)];
		}
	}
