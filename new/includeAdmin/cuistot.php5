	...
	function echoItem ($idCommande, $item, $quantite, $pret, $type, $xpath, $parent, $idmenu=''){
		...
		switch ($type) {
			case "Crepe":
				...
				if ($result->length != 0) {
					$trouve=true;
					echo " (Crêpe standard) <a href=\"cuistot/itempret.php?commande=".$idCommande."&item=".$linkitem."&pret=".$pret."&parent=".$parent."\"><img src=\"interf/livrer.png\"></a><ul>";
			...
			case "Perso":
				...
				if ($row4 != 0) {
					...
					$sucre = $row5["sucre"];
					echo " (".(($sucre)?"Crêpe":"Galette").") <a href=\"cuistot/itempret.php?commande=".$idCommande."&item=".$linkitem."&pret=".$pret."&parent=".$parent."\"><img src=\"interf/livrer.png\"></a><ul>";
					$k=0;
					while($row4){
